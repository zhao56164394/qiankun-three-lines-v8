...
def main():
    ...
    print(f'=== 八卦分治资金回测 v4 ===')
    print(f'  v3 → v4 修正:')
    print(f'    1. 重开 乾/离/兑 (实测 ret_30 +5%/+4%/+2%, 都赚钱)')
    print(f'    2. 关闭 艮 (实测 -13%, 25% 胜率)')
    print(f'    3. 放低 score 门槛 (坤/坎 score=0/1 也作候选)')
    print(f'    4. 跨 regime 统一 quality 排序 (按 ret_30 期望权重)')
    print(f'  策略池: 7 regime (除艮), quality 排序选 Top {MAX_PER_DAY}')
    print()
    ...
    for di, today in enumerate(all_dates):
        ...
        if today in df_by_date.groups:
            ...
            for ridx in today_idx_in_df:
                ...
                if quality <= 0: continue  # 关闭的 regime/score 直接跳过
                candidates.append({
    ...
    df_nav.to_csv(os.path.join(out_dir, 'capital_nav_v4.csv'), index=False, encoding='utf-8-sig')
    df_trades.to_csv(os.path.join(out_dir, 'capital_trades_v4.csv'), index=False, encoding='utf-8-sig')
    print(f'\n  写出: data_layer/data/results/capital_{{nav,trades}}_v4.csv')
