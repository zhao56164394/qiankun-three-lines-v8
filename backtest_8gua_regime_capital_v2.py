# -*- coding: utf-8 -*-
"""八卦分治资金回测 v2 — 修正 v1 的 5 个问题:
   1. 加 mf (main_force) / sanhu (retail) 数据
   2. 坎 v3 score 完全重写 (mf_5d<-50 / mf>100 / sanhu_5d<-100 / 大m=巽 / 大d=艮)
   3. 坤 v3 加 mf>100 (4 项 score)
   4. 坤 v3 卖点 bull+TS20 hybrid (持仓≥20日且 trend 从未到89 → 强卖)
   5. buy 用当日 close (跟文档统计一致, 不是 next_open)

底仓 20 万, 每日上限 10 只, 等资金 (2 万/只)
"""
...
def regime_buy_decide(mkt_y, mkt_d, mkt_m, stk_d, stk_m, stk_y,
    ...
    # 坤 v3: 巽日 + 9 避雷 + score≥2 (4 项: 大m=震, 大d=巽, mf>100, 股m=坎)
    if mkt_y == '000':
    ...
    # 艮 v3: 巽日, 单一避雷 (n=233 极薄, 简化保留)
    if mkt_y == '001':
        if stk_d != '011': return None
        return ('艮 v3', 1)

    # 坎 v3 (重写): 巽日 + 4 避雷 + score≥2 (5 项: 大m=巽, 大d=艮, mf_5d<-50, mf>100, sanhu_5d<-100)
    if mkt_y == '010':
        if stk_d != '011': return None
        # 4 硬避雷
        if mkt_m in {'100', '110'}: return None  # 大m=震/兑
        if stk_y == '111': return None  # 股y=乾
        if stk_m == '110': return None  # 股m=兑
        # 5 项 score
        score = 0
...
def main():
    ...
    print(f'=== 八卦分治资金回测 v2 ===')
    print(f'  v1 → v2 修正:')
    print(f'    1. 加 mf / sanhu 数据')
    print(f'    2. 坎 v3 score 完整 (mf_5d<-50 / mf>100 / sanhu_5d<-100)')
    print(f'    3. 坤 v3 加 mf>100 (4 项 score)')
    print(f'    4. 坤 v3 卖点 bull+TS20')
    print(f'    5. buy 当日 close (T+0)')
    print()
    ...
    df_nav.to_csv(os.path.join(out_dir, 'capital_nav_v2.csv'), index=False, encoding='utf-8-sig')
    df_trades.to_csv(os.path.join(out_dir, 'capital_trades_v2.csv'), index=False, encoding='utf-8-sig')
    print(f'\n  写出: data_layer/data/results/capital_{{nav,trades}}_v2.csv')
