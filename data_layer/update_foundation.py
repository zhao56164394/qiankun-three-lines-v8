...
# ============================================================
# 6. 宏观卦 (输出仅880KB，重算指标很快)
# ============================================================
def update_macro_bagua(new_cross_df=None):
    path = foundation_file('macro_bagua_daily.csv')
    last = _get_last_date(path)
    if last is None:
        print('  宏观卦不存在，需先运行: python data_layer/prepare_macro_bagua.py')
        return None

    cross_path = foundation_file('daily_cross_section.csv')
    cross_last = _get_last_date(cross_path)
    if cross_last is None or cross_last <= last:
        print(f'  宏观卦已是最新 ({last})')
        return None

    print(f'  宏观卦增量: {last} → {cross_last}...')

    from data_layer.prepare_macro_bagua import (
        _calc_macro_features, OUTPUT_COLUMNS, get_macro_params,
    )
    ...
    market = _calc_macro_features(combined_agg)
    market = _mark_segments(market)
    for col in OUTPUT_COLUMNS:
        if col not in market.columns:
            market[col] = pd.NA
    ...
    market.to_csv(path, index=False, encoding='utf-8-sig')
    print(f'  OK 宏观卦: → {market["date"].iloc[-1]} ({len(market)}行)')
    return market


# ============================================================
# 7. 个股卦 (1.3GB，用现有输出做warmup)
# ============================================================
