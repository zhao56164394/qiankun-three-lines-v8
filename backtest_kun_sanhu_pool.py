# -*- coding: utf-8 -*-
"""坤 + 散户线入池 v5: retail < -250 + F+B 双轨

入池条件: retail (散户线) < -250 (老 v3 标准)
出池: 满足 mkt_y=000 + stk_d=011 (基础事件)
排序: 优先 score=3, 次 score=2 中 sanhu_5d 最低 (F+B 双轨)
卖点: bull + TS20 + 60d 兜底
...
def run_backtest(K, df_e_today_picks, code_index, code_date_idx, code_arr, date_arr,
    ...
    # 把 picks 按日期分组
    picks_by_date = df_e_today_picks.set_index('date').to_dict('index')
...
def main():
    t0 = time.time()
    print('=== 坤 + sanhu<-250 入池 + F+B 双轨 资金回测 ===\n')
    ...
    df['sanhu_5d'] = df.groupby('code', sort=False)['retail'].transform(
        lambda s: s.rolling(5, min_periods=3).mean())
    # v5 入池条件: 散户线 < -250 (替代 trend 下穿 11)
    df['pool_enter'] = df['retail'] < -250
    print(f'  {len(df):,} 行')

    code_arr = df['code'].to_numpy()
    date_arr = df['date'].to_numpy()
    close_arr = df['close'].to_numpy().astype(np.float64)
    cross_arr = df['pool_enter'].to_numpy()  # 沿用 cross_arr 名字, 实际是 pool_enter
    ...
    # ============ F+B 双轨: 每日选 1 只 ============
    # 优先级 1: 当日 score=3 (或更高) 中 sanhu_5d 最低
    # 优先级 2: 当日 score=2 中 sanhu_5d 最低
    ...
    best['df_t'].to_csv(os.path.join(out_dir, 'kun_sanhu_pool_trades.csv'), index=False, encoding='utf-8-sig')
    best['df_n'].to_csv(os.path.join(out_dir, 'kun_sanhu_pool_nav.csv'), index=False, encoding='utf-8-sig')
    print(f'\n  写出 kun_sanhu_pool_{{trades,nav}}.csv')
