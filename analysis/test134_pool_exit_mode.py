...
测试 8 种出池机制:
  E0: retail >= 0 (当前)
  E1: 第 1 次巽日触发后即出池 (旧 v3 思路)
  E2: 永不主动出池, 只在新 retail<-250 时重置 enter_i
  E3: retail >= -50
  E4: retail >= -100
  E5: retail >= 50
  E6: 池子 30 天无新深度 (last_deepen 距离 > 30)
  E7: 池子 60 天无新深度
"""
...
def scan_events(arrays, exit_mode):
    """exit_mode: 'E0', 'E1', 'E2', 'E3', 'E4', 'E5', 'E6', 'E7'"""
    code_starts = arrays['starts']
    code_ends = arrays['ends']
    ...
    for ci in range(len(code_starts)):
        ...
        pool_last_deepen_i = -1  # E6/E7 用

        for i in range(LOOKBACK, n - MAX_HOLD - 1):
            gi = s + i

            # 入池: retail<-250 (E2 模式: 每次破 -250 都重置)
            if exit_mode == 'E2':
                # 永不主动出池, 但破新低重置 enter_i
                if retail[gi] < POOL_THR:
                    if not in_pool or retail[gi] < pool_min_retail:
                        if not in_pool:
                            in_pool = True
                        pool_enter_i = i
                        pool_min_retail = retail[gi]
                        pool_last_deepen_i = i
            else:
            ...
            # 主动出池
            if in_pool:
                if exit_mode == 'E0' and retail[gi] >= 0:
                    in_pool = False; continue
                elif exit_mode == 'E3' and retail[gi] >= -50:
                    in_pool = False; continue
                elif exit_mode == 'E4' and retail[gi] >= -100:
                    in_pool = False; continue
                elif exit_mode == 'E5' and retail[gi] >= 50:
                    in_pool = False; continue
                elif exit_mode == 'E6' and (i - pool_last_deepen_i) > 30:
                    in_pool = False; continue
                elif exit_mode == 'E7' and (i - pool_last_deepen_i) > 60:
                    in_pool = False; continue
...
def main():
    ...
    K = 5
    print(f'  {"方案":<28} {"事件":>6} {"信号天":>6} {"期末":>10} {"总收益":>9} {"年化":>9} {"MDD":>9} {"笔":>4} {"胜率":>6} {"均/笔":>7} {"平均仓":>7}')

    results = {}
    for label, mode in schemes.items():
        df_e = scan_events(arrays, mode)
        ...
        print(f'  {label:<28} {len(df_e):>6,} {n_sig_days:>6} ¥{r["final"]/1000:>8.0f}K '
              ...
              f'{r["n_trades"]:>4} {r["win"]:>5.1f}% {r["avg"]:>+5.2f}% {r["avg_pos"]:>5.1f}%')

    # 跨段对比
    print('\n=== 跨段对比 (按买入年份) ===\n')
    headers = list(schemes.keys())[:5]  # 只看前 5 个
    print(f'  {"年":<6}', end='')
    for h in headers:
        print(f' | {h[:14]:>14}', end='')
    print()
    ...
    for label in headers:
        t = results[label][0]['df_t']
        if len(t):
            all_years |= set(pd.to_datetime(t['buy_date']).dt.year.unique())
    for y in sorted(all_years):
        print(f'  {y:<6}', end='')
        for label in headers:
            t = results[label][0]['df_t']
            if len(t):
                t_y = t[pd.to_datetime(t['buy_date']).dt.year == y]
                if len(t_y):
                    n = len(t_y)
                    r = t_y['ret_pct'].mean()
                    w = (t_y['ret_pct']>0).mean()*100
                    print(f' | {n:>2}/{r:>+5.1f}/{w:>4.0f}%', end='')
                else:
                    print(f' | {"-":>14}', end='')
            else:
                print(f' | {"-":>14}', end='')
