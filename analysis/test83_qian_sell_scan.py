...
入场: 乾 regime + 巽日 + 6 项强避雷 + score ≥ 1 (个股 m_gua=坎 OR 个股 y_gua=坎)
注: 两条好规律重叠在"坎卦", score>=1 实际是命中任一坎条件
"""
...
def main():
    ...
    for ci in range(len(code_starts)):
        ...
        for i in range(LOOKBACK, e - s - MAX_HOLD - 1):
            gi = s + i
            if mkt_y_arr[gi] != '111': continue  # 乾 regime
    ...
    df_en['seg'] = ''
    for w_name, (a, b) in [('w2_2019', ('2019-01-01', '2020-01-01')),
    ...
    print(f'\n## 卖点对比 (n={len(df_en)})')
    print(f'  {"机制":<14} {"全期望%":>8} {"胜率%":>7} {"持仓":>5} {"主升期望%":>10} {"假期望%":>9} {"w2":>6} {"w3":>6} {"w4":>6} {"w5":>6} {"w6":>6} {"w7":>6}')
    ...
    for sn in sells_to_show:
        ...
        rets = []
        for sg in ['w2_2019', 'w3_2020', 'w4_2021', 'w5_2022', 'w6_2023_24', 'w7_2025_26']:
            sg_sub = sub[sub['seg'] == sg]
            r = sg_sub[rcol].mean() if len(sg_sub) > 50 else float('nan')
            rets.append(r)
        print(f'  {sn:<14} {ret:>+7.2f} {win:>6.1f} {hold:>5.1f} {zsl:>+9.2f} {fake:>+8.2f} {rets[0]:>+5.2f} {rets[1]:>+5.2f} {rets[2]:>+5.2f} {rets[3]:>+5.2f} {rets[4]:>+5.2f} {rets[5]:>+5.2f}')
