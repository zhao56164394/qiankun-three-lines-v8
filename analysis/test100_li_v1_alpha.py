# -*- coding: utf-8 -*-
"""离 v1 vs 全市场 alpha 检查

入场: 离 regime + 坤触发 + 5 项避雷 (无 score)
卖点: bull (跨 regime 一致)
...
def main():
    ...
    for ci in range(len(code_starts)):
        ...
        for i in range(LOOKBACK, n - EVAL_WIN):
            gi = s + i
            if mkt_y_arr[gi] != '101': continue  # 离

            ret_30 = (cl[i+EVAL_WIN] / cl[i] - 1) * 100
            all_market.append({'date': date_arr[gi], 'ret_30': ret_30})

            # v1 入场
            if stk_d_arr[gi] != '000': continue  # 坤
            if mkt_d_arr[gi] == '101': continue
            if stk_m_arr[gi] in {'011', '001', '101'}: continue
            if stk_y_arr[gi] == '011': continue
    ...
    print(f'  全市场 (离 regime 内): {len(df_all):,}')
    print(f'  v1 事件: {len(df_v1):,}')

    print(f'\n## v1 vs 全市场 alpha (30 日)')
    print(f'  {"窗口":<14} {"全市场n":>8} {"全市场":>7} {"v1 n":>7} {"v1 30日":>8} {"alpha 30":>9}  {"v1 bull":>8}')
    for w_name, lo, hi in WINDOWS:
        ...
        print(f'  {w_name:<14} {len(df_all_seg):>8,} {all_ret:>+6.2f} {len(df_v1_seg):>7,} '
