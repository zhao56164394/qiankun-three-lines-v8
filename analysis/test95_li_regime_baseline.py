# -*- coding: utf-8 -*-
"""阶段 2: 离 regime (mkt_y=101) baseline + 主升浪事件分布 + 8 触发卦同扫

离 = y_pos=1 + y_spd=0 + y_acc=1
含义: 长期高 + 中期低 + 短期高 — 大涨后回调中再反弹 / 中继回升期
   skill 表 baseline +2.54%, 是 8 regime 中第三好的环境
"""
...
def main():
    ...
    # ===== 8 触发卦 baseline 同扫 =====
    print(f'\n=== 离 regime ({REGIME_Y}) — 8 触发卦 baseline 同扫 ===')
    all_events = []
    ...
    print(f'  离 regime 总事件 (任何卦): {len(df_all):,}')
    print(f'  离 regime 全期期望: {df_all["ret_30"].mean():+.2f}%')

    # 按段分布看跨段
    print(f'\n  按段分布 (任何触发卦):')
    print(f'  {"seg":<14} {"n":>8} {"30日%":>7}')
    seg_baselines = {}
    for w in WINDOWS:
        sub = df_all[df_all['seg'] == w[0]]
        if len(sub) > 0:
            r = sub['ret_30'].mean()
            seg_baselines[w[0]] = r
            print(f'  {w[0]:<14} {len(sub):>8,} {r:>+6.2f}')

    print(f'\n  按触发卦分组 (含 walk-forward):')
    ...
    # ===== 主升浪事件分布 =====
    print(f'\n\n=== 离 regime 主升浪 (≥{QIAN_RUN}日 乾连续) 事件分布 ===')
    runs = []
    for ci in range(len(code_starts)):
        s = code_starts[ci]; e = code_ends[ci]
        if e - s < 5: continue
