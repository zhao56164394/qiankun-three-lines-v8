# -*- coding: utf-8 -*-
"""阶段 2: 坎 regime (mkt_y=010) baseline + 主升浪事件分布

目标:
  - 验证 baseline +3.41%, 4/4 段稳
  - 主升浪 (≥10 日) 事件按段分布, 确认跨段足够
  - 起点前一日卦象分布 (验证巽日入口规律是否在坎 regime 也成立)
"""
...
def main():
    ...
    # ===== 坎 regime 巽日 baseline =====
    print(f'\n=== 坎 regime ({REGIME_Y}) 巽日 baseline ===')
    events = []
    ...
    # ===== 主升浪事件分布 =====
    print(f'\n=== 坎 regime 主升浪 (≥{QIAN_RUN}日 乾连续) 事件分布 ===')
    runs = []
    for ci in range(len(code_starts)):
        s = code_starts[ci]; e = code_ends[ci]
        if e - s < 5: continue
