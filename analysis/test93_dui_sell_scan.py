...
入场: 兑 regime (mkt_y=110) + 坤触发 (stk_d=000) + 5 项强避雷
  - 大d=011 巽
  - 股m=001 艮 / 011 巽 / 101 离 / 111 乾

样本只有 2 段 (w2_2019 / w5_2022), 任何 magic number 都是切片
卖点目标: bull 主路 + 跨 regime 一致, 避免过拟合
"""
...
def main():
    ...
    for ci in range(len(code_starts)):
        ...
        for i in range(LOOKBACK, e - s - MAX_HOLD - 1):
            ...
            if stk_d_arr[gi] != '000': continue  # 坤触发
            # 强避雷
    ...
    print(f'\n## 卖点对比')
    print(f'  {"卖点":<14} {"全期望":>7} {"胜":>5} {"持仓":>5} {"主升期":>7} {"假期":>7}  {"w2":>6} {"w5":>6}  {"段稳":>5}')
    for sn in sells_to_eval:
        ...
        print(f'  {sn:<14} {ret:>+6.2f} {win:>5.1f} {hold:>5.1f} {zsl:>+7.2f} {fake:>+6.2f}  '
              f'{rets[0]:>+5.2f} {rets[1]:>+5.2f}  {n_pos}/{n_seg}')
