# -*- coding: utf-8 -*-
"""阶段 2: 震 regime (mkt_y=100) baseline + 主升浪事件分布 + 8 触发卦同扫

震 = y_pos=1 + y_spd=0 + y_acc=0
含义: 长期高 + 中期低 + 短期低 — 高位深度回调期 / 中期顶部回落
   skill 表 baseline +1.16%
"""
...
def main():
    ...
    # ===== 8 触发卦 baseline 同扫 =====
    print(f'\n=== 震 regime ({REGIME_Y}) — 8 触发卦 baseline 同扫 ===')
    all_events = []
    ...
    print(f'  震 regime 总事件 (任何卦): {len(df_all):,}')
    print(f'  震 regime 全期期望: {df_all["ret_30"].mean():+.2f}%')
    ...
    # ===== 主升浪事件分布 =====
    print(f'\n\n=== 震 regime 主升浪 (≥{QIAN_RUN}日 乾连续) 事件分布 ===')
    runs = []
    for ci in range(len(code_starts)):
        s = code_starts[ci]; e = code_ends[ci]
        if e - s < 5: continue
