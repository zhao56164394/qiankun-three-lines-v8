# -*- coding: utf-8 -*-
"""阶段 4: 兑 regime 反向避雷扫描

样本只有 2 段: w2_2019 (大牛, +2.79% baseline) / w5_2022 (大熊, -11.11% baseline)
触发: 000 坤 (alpha vs 全市场 +1.48%, 2/2 段都正)

判定:
  2/2 段都 fail (lift ≤ -1) → 强避雷
  2/2 段都 pass (lift ≥ +1) → 强好规律
  1/2 段 → 弱信号 (考虑切片可能)
"""
...
QIAN_RUN = 10
REGIME_Y = '110'  # 兑
TRIGGER_GUA = '000'  # 坤 (兑 regime 内唯一 2 段都正 alpha)
...
def main():
    ...
    def fmt_seg(seg_results):
        ...
        for w in WINDOWS:
            ...
            if r is None: s.append(f'{w[0][:2]}=NA')
            else: s.append(f'{w[0][:2]}={r[2]:+.1f}')
        return ' '.join(s)

    print(f'\n## ★★ 强避雷 (2/2 段都 fail, lift ≤ -1)')
    if not avoid_strong: print('  无')
    for col, state, label, n, ret, seg in avoid_strong:
        print(f'  {label:<14} 全n {n:>5,} 全期望 {ret:>+5.2f}%  {fmt_seg(seg)}')

    print(f'\n## ★★ 强好规律 (2/2 段都 pass, lift ≥ +1)')
    if not good_strong: print('  无')
    for col, state, label, n, ret, seg in good_strong:
        print(f'  {label:<14} 全n {n:>5,} 全期望 {ret:>+5.2f}%  {fmt_seg(seg)}')

    # 列出所有看到的 (lift 排序), 不只是强信号
    print(f'\n## 全部单维 lift 排序 (供肉眼判断)')
    all_results = []
    for col, label_short in [('mkt_d', '大d'), ('mkt_m', '大m'),
                              ('stk_y', '股y'), ('stk_m', '股m')]:
        for state in GUAS:
            ...
            n_full = len(sub)
            avg_lift = np.nanmean([r[2] for r in seg_results.values() if r is not None])
            all_results.append((label, n_full, ret_full, avg_lift, seg_results))

    all_results.sort(key=lambda x: x[3], reverse=True)
    print(f'\n  Top 8 (好):')
    for label, n, ret, avg_lift, seg in all_results[:8]:
        print(f'  {label:<14} 全n {n:>5,} 全期望 {ret:>+5.2f}% 平均lift {avg_lift:>+5.2f}  {fmt_seg(seg)}')
