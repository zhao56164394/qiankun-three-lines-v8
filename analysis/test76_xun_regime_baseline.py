# -*- coding: utf-8 -*-
"""阶段 2: 巽 regime (mkt_y=011) baseline + 主升浪事件分布

skill baseline 表显示 -3.15%, 跨段稳定性差
本脚本验证实际数字, 决定后续是否进 v3 全流程或仅 case study
"""
...
def main():
    ...
    # ===== 巽 regime 巽日 baseline =====
    print(f'\n=== 巽 regime ({REGIME_Y}) 巽日 baseline ===')
    events = []
    ...
    # ===== 主升浪事件分布 =====
    print(f'\n=== 巽 regime 主升浪 (≥{QIAN_RUN}日 乾连续) 事件分布 ===')
    runs = []
    for ci in range(len(code_starts)):
        s = code_starts[ci]; e = code_ends[ci]
        if e - s < 5: continue
