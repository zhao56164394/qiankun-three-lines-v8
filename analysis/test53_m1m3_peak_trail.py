# -*- coding: utf-8 -*-
"""Step 32 — M1∩trail / M3∩trail 重新实现 (peak 重置语义)

正确语义:
  M1∩trail (乾切∩trail):
    - 第一次出现"乾切" (上日 d=111, 今日 ≠111): peak = 上日 close (即乾切前一天的乾顶 close)
    - 之后跟踪: cur close < peak * (1 - trail/100) 触发卖
    - 期间如果 d_gua 又变回 111 然后再"乾切": peak 重置为新乾切的上日 close
    - 60 日内未触发 → 强卖

  M3∩trail (下穿89∩trail):
    - 第一次"下穿89" (上日 d_trend>89, 今日 ≤89): peak = 上日 close
    - 之后跟踪: cur close < peak * (1 - trail/100) 触发卖
    - 期间 d_trend 又涨过89 再下穿: peak 重置
    - 60 日内未触发 → 强卖

trail 多变量: 5/8/10/12/15/20/25
"""
import os
import sys
import io
...
def sell_m1_peak_trail(buy_idx, gua, cl, end_idx, trail_pct):
    """乾切跟踪止损: 每次乾切后 peak=上日close, 跌过 trail% 卖, 期间新乾切重置 peak"""
    n = len(gua)
    end = min(end_idx, n - 1)
    peak = None  # 当前活跃 peak (None = 还没出现过乾切)
    for k in range(buy_idx + 1, end + 1):
        # 检测乾切
        if gua[k-1] == '111' and gua[k] != '111':
            # 重置 peak (上日 close = cl[k-1])
            peak = cl[k-1]
        # 跟踪止损 (只有 peak 已设)
        if peak is not None:
            if cl[k] < peak * (1 - trail_pct / 100.0):
                return k, 'qian_trail'
    return end, 'timeout'


def sell_m3_peak_trail(buy_idx, td, cl, end_idx, trail_pct):
    """下穿89 跟踪止损: 每次下穿89 后 peak=上日close, 跌过 trail% 卖, 期间新下穿重置"""
    n = len(td)
    end = min(end_idx, n - 1)
    peak = None
    for k in range(buy_idx + 1, end + 1):
...
def main():
    ...
    # 模拟
    print(f'\n=== 模拟 M1∩peak_trail / M3∩peak_trail × {len(TRAIL_PCTS)} ===')
    t1 = time.time()
    results = {'B2_bull': []}
    for trail in TRAIL_PCTS:
        results[f'M1_peak_{trail}'] = []
        results[f'M3_peak_{trail}'] = []
    ...
    for gi, ci, score in buy_events:
        ...
        # B2 bull
        sl, ex = sell_b2_bull(local_buy, td_seg, cl_seg, max_end)
        results['B2_bull'].append({**common, 'hold': sl-local_buy,
                                     'ret': (cl_seg[sl]/buy_close-1)*100, 'exit': ex})

        for trail in TRAIL_PCTS:
            sl, ex = sell_m1_peak_trail(local_buy, gua_seg, cl_seg, max_end, trail)
            results[f'M1_peak_{trail}'].append({**common, 'hold': sl-local_buy,
                                                  'ret': (cl_seg[sl]/buy_close-1)*100, 'exit': ex})
            sl, ex = sell_m3_peak_trail(local_buy, td_seg, cl_seg, max_end, trail)
            results[f'M3_peak_{trail}'].append({**common, 'hold': sl-local_buy,
                                                  'ret': (cl_seg[sl]/buy_close-1)*100, 'exit': ex})

    print(f'  完成 {time.time()-t1:.1f}s')

    # M1 peak trail
    print(f'\n## M1 ∩ peak_trail (乾切后峰值跟踪止损, 新乾切重置 peak)')
    print(f'  {"trail":<8} {"期望%":>7} {"中位%":>7} {"胜率":>6} {"均持仓":>6} {"主升期望":>9} {"假期望":>8} {"trail触发":>9}')
    print('  ' + '-' * 78)
    for trail in TRAIL_PCTS:
        d = pd.DataFrame(results[f'M1_peak_{trail}'])
        ret_m = d['ret'].mean(); ret_med = d['ret'].median()
        win = (d['ret']>0).mean()*100
        hold = d['hold'].mean()
        zsl = d[d['is_zsl']]; fake = d[~d['is_zsl']]
        trig = (d['exit']=='qian_trail').mean()*100
        print(f'  trail={trail}%  {ret_m:>+6.2f}% {ret_med:>+6.2f}% {win:>5.1f}% {hold:>5.1f} '
              f'{zsl["ret"].mean():>+7.2f}% {fake["ret"].mean():>+7.2f}% {trig:>7.1f}%')

    # M3 peak trail
    print(f'\n## M3 ∩ peak_trail (下穿89后峰值跟踪止损, 新下穿重置 peak)')
    print(f'  {"trail":<8} {"期望%":>7} {"中位%":>7} {"胜率":>6} {"均持仓":>6} {"主升期望":>9} {"假期望":>8} {"trail触发":>9}')
    print('  ' + '-' * 78)
    for trail in TRAIL_PCTS:
        d = pd.DataFrame(results[f'M3_peak_{trail}'])
        ret_m = d['ret'].mean(); ret_med = d['ret'].median()
        win = (d['ret']>0).mean()*100
        hold = d['hold'].mean()
        zsl = d[d['is_zsl']]; fake = d[~d['is_zsl']]
        trig = (d['exit']=='cross89_trail').mean()*100
        print(f'  trail={trail}%  {ret_m:>+6.2f}% {ret_med:>+6.2f}% {win:>5.1f}% {hold:>5.1f} '
              f'{zsl["ret"].mean():>+7.2f}% {fake["ret"].mean():>+7.2f}% {trig:>7.1f}%')
    ...
    print(f'\n## B2 bull (基准)')
    print(f'  期望 {d["ret"].mean():+.2f}%, 胜率 {(d["ret"]>0).mean()*100:.1f}%, 持仓 {d["hold"].mean():.1f}日')

    # walk-forward
    print(f'\n## walk-forward 各段 期望% (B2 + M1_peak × 4 + M3_peak × 4)')
    cols = ['B2_bull', 'M1_peak_5', 'M1_peak_10', 'M1_peak_15', 'M1_peak_20',
            'M3_peak_5', 'M3_peak_10', 'M3_peak_15', 'M3_peak_20']
    print(f'  {"段":<14}', end='')
    for c in cols:
        print(f' {c[:11]:>11}', end='')
    print()
