# -*- coding: utf-8 -*-
"""阶段 4: 艮 regime 反向避雷 (w2_2019 + w4_2021 跨段)

样本: 艮 regime 全期巽日 (2,164), 跨 2 段:
  w2_2019 (438) — baseline -4.63%  ← 大熊段
  w4_2021 (1713) — baseline +7.12% ← 抱团段

避雷判定:
  - 段内 ret 差 baseline ≥ 1% 算 fail
  - 两段都 fail → 真避雷
  - 仅一段 fail → 标记 (case study 用, 不可硬用)
"""
import os
import sys
import io
...
def main():
    ...
    for w in WINDOWS:
        ...
        print(f'  baseline {w[0]:<10} n={len(seg):>5}  期望 {seg_baselines[w[0]]:>+5.2f}%')

    # 单维候选扫描
    print(f'\n## 跨段稳定避雷扫描 (≥{MIN_N_SEG} n/段, lift < {LIFT_FAIL}%)')
    avoid_strong = []   # 两段都 fail
    avoid_weak = []     # 单段 fail (仅 w4)
    good_strong = []    # 两段都 pass
    good_weak = []      # 单段 pass
    ...
    for col, label_short in [('mkt_d', '大d'), ('mkt_m', '大m'),
        ...
        for state in GUAS:
            ...
            elif n_valid >= 1 and n_fail >= 1 and n_pass == 0:
            ...
            elif n_valid >= 1 and n_pass >= 1 and n_fail == 0:
    ...
    if avoid_strong:
        ...
        for w in WINDOWS:
            ...
            print(f'    {w[0]:<10} 全 {b:>+5.2f}%, 避雷后 {k:>+5.2f}% ({len(seg_k):>4}), lift {diff:>+5.2f} {mark}')
