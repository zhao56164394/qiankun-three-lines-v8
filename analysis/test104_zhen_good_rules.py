# -*- coding: utf-8 -*-
"""阶段 5: 震 regime 好规律 IS/OOS 软排名

入场池: 震 regime + 坎触发 + 弱避雷 (3 项)
  弱: 大d=101离, 大d=111乾, 股y=111乾

IS = w3 + w4 + w5 (3 段, 跨抱团/熊)
OOS = w6 + w7 (2 段)
注: w2 样本只 230, 不够独立验证, 跟 IS 一起算
...
WIN_IS = [('w3', '2020-01-01', '2021-01-01'),
...
def main():
    ...
    for ci in range(len(code_starts)):
        ...
        for i in range(LOOKBACK, n - EVAL_WIN):
            ...
            # 避雷 (3 项弱避雷)
            if mkt_d_arr[gi] in {'101', '111'}: continue  # 大d=离/乾
            if stk_y_arr[gi] == '111': continue  # 股y=乾
    ...
    df_e['period'] = df_e['seg'].apply(lambda s: 'IS' if s in {'w3', 'w4', 'w5'} else 'OOS')

    print(f'  事件: {len(df_e):,}')
    is_data = df_e[df_e['period'] == 'IS']; oos_data = df_e[df_e['period'] == 'OOS']
    print(f'  IS (w3-w5): n={len(is_data):,}, 期望 {is_data["ret_30"].mean():+.2f}%')
