...
入场逻辑 (每日只买 1 只):
  优先级 1 (B): score >= 3 → 选 score=3 中 sanhu_5d 最低
  优先级 2 (F): score = 2 → 选 score=2 中 sanhu_5d 最低
  没有 score >= 2 信号 → 不买
...
def main():
    ...
    best['df_t'].to_csv(os.path.join(out_dir, 'kun_pool_fb_trades.csv'), index=False, encoding='utf-8-sig')
    best['df_n'].to_csv(os.path.join(out_dir, 'kun_pool_fb_nav.csv'), index=False, encoding='utf-8-sig')
    print(f'\n  写出 kun_pool_fb_{{trades,nav}}.csv')
